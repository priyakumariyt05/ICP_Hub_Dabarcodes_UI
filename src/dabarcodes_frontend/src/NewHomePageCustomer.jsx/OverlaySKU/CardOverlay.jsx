...
//   const [tableData, setTableData] = useState(redemptions);
    ...
    <div className="p-8 max-w-4xl  font-roboto lg:m-4">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-start justify-between">
        <div className="lg:w-2/3">
          ...
          <div className="mt-4">
             <p className="text-blue-600 text-xl mt-1 font-roboto">${product.originalPrice} <span className="text-sm text-gray-500 line-through">${product.discountedPrice}</span></p>
       ...
       ${product.discountLabel} Off
            ...
            <p className="text-red-600 text-medium font-roboto py-2">Valid until {product.validUntil}</p>
            <p className="text-sm">Promoted By: <br /> {product.promoter}</p>
            <p className="text-sm">+3 others</p>
          </div>

          {/* Product Description */}
          <p className="text-gray-700 mb-4 mt-4 w-96">
            {product.description}
          </p>
  ...
  <div className="flex">
    ...
    <div className="ml-6 w-1/2"> {/* Add margin-left here */}
      <p>{product.brand}</p>
      <p>{product.flavor}</p>
